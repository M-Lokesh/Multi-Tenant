...
public class JWTTokenGeneratorFilter extends OncePerRequestFilter {
    ...
    public JWTTokenGeneratorFilter(OrganizationService organizationService) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (null != authentication) {
            ...
            if (null != env) {
                String orgCode = request.getHeader(ORGANIZATION_HEADER);

                if (orgCode != null && !orgCode.isBlank()) {
                    organizationService.getOrganizationByCode(orgCode)
                        .ifPresent(org -> TenantContext.setOrganization(org.getId()));
                }
                String secret = env.getProperty(ApplicationConstants.JWT_SECRET_KEY,
                        ApplicationConstants.JWT_SECRET_DEFAULT_VALUE);
                SecretKey secretKey = Keys.hmacShaKeyFor(secret.getBytes(StandardCharsets.UTF_8));
                String jwt = Jwts.builder().issuer("Tenant").subject("JWT Token")
